  const {createClient} = supabase
    const supabaseUrl = 'https://xhcxtehkrpsdepubnksy.supabase.co';
    const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhoY3h0ZWhrcnBzZGVwdWJua3N5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjkyNTUxMDgsImV4cCI6MjA0NDgzMTEwOH0.sMTgiJmU0u4raSPAHdbZZfr3IxACHvMVbgWtehJU6l4';
    ...
    // part-number-detail
    const partNumberDetailCall =(key, partNumber, partName, partName2) =>{
        mainTab.style.display = 'none';
        PartsDetail.style.display = 'none';
        makeDetail.style.display = 'none';
    ...
    document.getElementById("interchangeForm").addEventListener("submit", function (event) {
        event.preventDefault();
        const oem_part = this.oem_part.value.trim();
        const oem_name = this.oem_name.value.trim();
        const part_type = this.part_type.value.trim();
        searchInterchange(oem_part, oem_name, part_type);
    });
